// Firework Color Definitions
const colors = [
	"#ff6f91", "#ff9671", "#ffc75f", "#f9f871", "#ff4c4c", "#ffcc00"
  ];
  const letters = "I LOVE YOU"; // Message for fireworks
  let letterIndex = 0; // Letter index
  
  // Get the next letter from the message
  function getRandomLetter() {
	const letter = letters.charAt(letterIndex); // Get the current letter
	letterIndex = (letterIndex + 1) % letters.length; // Move to next letter, loop back at the end
	return letter;
  }
  ...
  function animateParticle(el, isSparkle) {
	...
	const distance = anime.random(100, 200); // Distance for spread
	const duration = anime.random(1200, 2000); // Duration for effect
	const fallDistance = anime.random(20, 80); // Gravity effect for realism
	...
	.add({
	  translateY: `+=${fallDistance}px`, // Gravity effect
	  opacity: [0.9, 0],
	  easing: "easeInCubic",
	  duration: duration / 2
	});
  ...
  // Add click listener for fireworks
  ...
  // Handle the "Next Page" button to navigate to the next page
  document.getElementById('nextPage').addEventListener('click', () => {
	window.location.href = 'hand.html'; // Change 'nextpage.html' to your next page URL
  });
  
  // Handle the "Play Music" button to play/pause the music
